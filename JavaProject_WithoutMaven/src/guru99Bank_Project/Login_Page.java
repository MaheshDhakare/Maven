		...
		if (obj.findElement(UserName).isDisplayed()) {
			...
			if (obj.findElement(Password).isDisplayed()) {
				...
				if (obj.findElement(loginButton).isDisplayed()) {
					...
					WebElement validateLogin = driver.findElement(By.xpath("//marquee[@class='heading3']"));
					
					if (validateLogin.isDisplayed()) {
						System.out.println("Login to bank page successfully");
						
					} else {
						System.out.println("Login to bank page un-successfully");
					}
