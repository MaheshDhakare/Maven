...
public class Assignment_19_VerifyPartOfValueFromDropdownStoredInArray2 {
	
	XSSFWorkbook workBook;
	XSSFSheet workSheet;
	static int rowNum = 2;
	static String dropdownAttribute = "Severity";
				...
				objBussPro.clickOn(objBussPro.reportIsstab);
				 ...
				 if(objBussPro.getDropdownvalueInArray(dropdownAttribute).length != 0) {
					 ...
					 for (int cnt = 0; cnt < dropdownvalueInArray.length; cnt++) {
						 ...
						 objBussPro.verifyDropdownValue(objBussPro.selectSeverity, dropdownvalueInArray[cnt]);
