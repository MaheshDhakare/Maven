...
public class Assignment_19_VerifyPartOfValueFromDropdownStoredInArray {
	
	XSSFWorkbook workBook;
	XSSFSheet workSheet;
	static int rowNum = 2;
	static String dropdownAttribute = "Priority"; //update the dropdown value here and also update selectPriority in if condition of For.
	

public static void main(String[] args) throws IOException {
	
	System.out.println("===================Assignment_19_VerifyPartOfValueFromDropdownStoredInArray=================== ");
		...
		if (objBussPro.validedWebElement(objBussPro.logoutButton)) {
			...
			if (objBussPro.validedWebElement(objBussPro.reportIsstab)) {
				...
				objBussPro.clickOn(objBussPro.reportIsstab);
				 ...
				 if(objBussPro.getDropdownvalueInArray(dropdownAttribute).length != 0) {
					 
					 String[] dropdownvalueInArray = objBussPro.getDropdownvalueInArray(dropdownAttribute);
					 
					 for (int cnt = 0; cnt < dropdownvalueInArray.length; cnt++) {
						
						 if (objBussPro.verifyDropdownValue(objBussPro.selectPriority, dropdownvalueInArray[cnt]))
						 {
								
							 System.out.println("'" + dropdownvalueInArray[cnt] + "'"+ " is available in the Dropdown and available at index " + "'" + cnt + "'");

						} else
						{
							System.out.println("'"+dropdownvalueInArray[cnt]+"'"+" is not available in the Dropdown");
//							objBussPro.setCellvalue(rowNum, 12, "Failed");
//							objBussPro.setCellvalue(rowNum, 13, dropdownvalueInArray[cnt] +"value is not available in the Dropdown");
//							objBussPro.closeBrowser();
					...
					System.out.println("There are no values available ubder the dropdown " + "'"+dropdownAttribute+"'" );
				}
